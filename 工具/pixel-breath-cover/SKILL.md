...
以下完整格式只用于创建或更新 `发布清单.md`，或用户主动要求完整封面方案。明确的 prompt-only 请求只返回其中的完整单段 Prompt；反向分析请求按工作流第 5 步输出。
...
以下清单适用于具体选题、完整封面方案和发布清单分支。纯反向分支只检查：准确提炼参考图的画幅、信息层级、标题几何、人物比例、锚点逻辑、色彩与灯光；没有迁移参考人物身份、账号文案、品牌资产、具体道具或精确构图；并已按用户要求输出视觉语法或通用反向 Prompt。
