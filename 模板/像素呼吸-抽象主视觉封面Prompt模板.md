# 像素呼吸｜抽象主视觉封面 Prompt 模板（备用）

> **备用路线，不再作为默认封面。** 当前默认规则见 `工具/pixel-breath-cover/SKILL.md`，使用本人参考照建立人物识别。只有用户明确要求“抽象”或“无人物”时，才使用本模板。

## 一句话原则

使用本备用路线时，每期只讲一个概念，主题大字位于画面**几何中心**；英文副题紧贴在下；背景只承担情绪和隐喻，并且每期必须更换“视觉母题”。
