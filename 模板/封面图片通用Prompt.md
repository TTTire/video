...
> **历史参考，不作为当前默认封面。** 当前默认规则见 `工具/pixel-breath-cover/SKILL.md`：3:4 竖版、本人参考照、顶部超粗标题、一个主题物件或场景冲突。以下旧版 4:3 红黄漫画拼贴只在用户明确点名时使用；抽象无人物方向则使用 `模板/像素呼吸-抽象主视觉封面Prompt模板.md`。
